import pytest
from app.records_room.rag import VectorRAG  # ✅ Fixed import
...
def test_rag_ingest_and_search(tmp_path):
    ...
    file.write_text("The Mummy Resort is ancient and mysterious, located in the heart of the desert.")
    
    # Initialize VectorRAG with HuggingFace embeddings
    rag = VectorRAG(str(tmp_path / ".rag_store"), "test_collection")  # ✅ Fixed class name
...
def test_rag_multiple_documents(tmp_path):
    ...
    (folder / "vampires.txt").write_text("Vampire Manor offers eternal luxury with coffin suites.")
    (folder / "zombies.txt").write_text("Zombie Bed & Breakfast serves brains for breakfast.")
    (folder / "mummies.txt").write_text("The Mummy Resort wraps you in ancient comfort.")
...
def test_rag_search_no_results(tmp_path):
    ...
    assert result["ok"] is True
    # Should still return results (semantic search may find loosely related content)
    # but they won't be very relevant
