...
class TestContextOverlap:
    ...
    def test_unrelated_text_low_overlap(self, detector):
        text = "Quantum physics explains particle behavior at subatomic scales."
        contexts = ["The vampire manor has gothic architecture and dark hallways."]
        score = detector._compute_context_overlap(text, contexts)
        assert score < 0.3
...
class TestSourceAttribution:
    def test_grounded_response(self, detector):
        contexts = ["The Vampire Manor features eternal night ambiance and gothic towers."]
        response = "The Vampire Manor features eternal night ambiance. It has gothic towers."
        score = detector._compute_source_attribution(response, contexts)
        assert score > 0.5

    def test_ungrounded_response(self, detector):
        contexts = ["The Vampire Manor features eternal night ambiance and gothic towers."]
        response = "Quantum computers leverage superposition for parallel computation. Neural networks process data through layered architectures."
        score = detector._compute_source_attribution(response, contexts)
        assert score < 0.3
...
class TestScoreResponse:
    def test_high_confidence_identical(self, detector):
        text = "The Vampire Manor has dark gothic hallways and eternal night ambiance."
        contexts = [text]
        result = detector.score_response(text, contexts, "Tell me about Vampire Manor")
        assert result.level == ConfidenceLevel.HIGH
        assert result.overall_score >= 0.7

    def test_low_confidence_unrelated(self, detector):
        text = "Quantum computing leverages superposition for parallel computation across multiple dimensions."
        contexts = ["The Vampire Manor features eternal night ambiance and gothic towers."]
        result = detector.score_response(text, contexts, "Tell me about Vampire Manor")
